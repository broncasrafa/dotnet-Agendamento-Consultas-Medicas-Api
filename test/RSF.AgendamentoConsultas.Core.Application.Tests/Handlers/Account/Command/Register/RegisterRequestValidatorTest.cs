...
public class RegisterRequestValidatorTest
{
    ...
    [Theory(DisplayName = "Deve retornar erro ao validar o TipoAcesso")]
    [InlineData(null, "O Tipo de Acesso é obrigatório, não deve ser nulo ou vazio")]
    [InlineData("", "O Tipo de Acesso é obrigatório, não deve ser nulo ou vazio")]
    [InlineData("Teste", "O Tipo de Acesso deve ser 'Administrador' ou 'Consultor'.")]
    [InlineData("Paciente", "O Tipo de Acesso deve ser 'Administrador' ou 'Consultor'.")]
    public void Validate_ShouldHaveError_When_TipoAcesso_IsInvalid(string tipoAcesso, string expectedMessage)
    {
        //Arrange
        var request = new RegisterRequest(
                NomeCompleto: _faker.Person.FullName,
